...
def menu():
    ...
    disp.create_text(disp.winfo_width()/2, disp.winfo_height()/2 + 20,
                     text="Create Nick Name", fill="white", font=otherFont)
    global name
    texto=data.get()
    if len (texto) >= 0:
        name = texto
...
data = StringVar()
textField = Entry(disp,textvariable=data).place(x=320,y=450)
savename=Button(disp, text="Set Nick Name",command=menu).place(x=450,y=450)

playbut=Button(disp, text=" Play ", font=21, command=startGame).place(x=350,y=480)
